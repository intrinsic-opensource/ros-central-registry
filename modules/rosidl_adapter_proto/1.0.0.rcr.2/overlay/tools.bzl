...
def _strip_virtual_imports(path):
    if "_virtual_imports/" not in path:
        return path
    parts = path.split("/")
    for i in range(len(parts)):
        if parts[i] == "_virtual_imports":
...
def merge_proto_infos(ctx, name, deps, srcs = []):
    ...
    if srcs:
        # We are in the aspect
        direct_protos = srcs
        proto_deps = [
            dep[RosProtoInfo].proto_info
            for dep in deps
            if RosProtoInfo in dep
        ]
    else:
        # We are in the rule (proto_ros_library)
        direct_protos = []
        proto_deps = []
        for dep in deps:
            if RosProtoInfo in dep:
                direct_protos.extend(dep[RosProtoInfo].proto_info.direct_sources)
                proto_deps.extend(dep[RosProtoInfo].proto_deps)
    ...
    for proto in direct_protos:
        ...
        pkg_type_name = _strip_virtual_imports(pkg_type_name)
    ...
    if not virtual_srcs:
        ctx.actions.write(output = descriptor_set, content = "")
    else:
