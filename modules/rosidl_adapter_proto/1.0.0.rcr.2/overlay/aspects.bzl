...
load("@rosidl_parser//:defs.bzl", "RosIdlInfo", "RosInterfaceInfo", "generate_sources")
...
def _rosidl_adapter_proto_aspect_impl(target, ctx):
    ...
    # Generate source files file for the current mode.
    hdrs, srcs, _ = generate_sources(
        target = target,
        ctx = ctx,
        executable = ctx.executable._proto_generator,
        mnemonic = "ProtoGeneration",
        input_idls = [target[RosIdlInfo].idl],
        input_templates = ctx.attr._proto_templates[DefaultInfo].files.to_list(),
        input_templates_dir = ctx.attr._proto_templates[DefaultInfo].files.to_list()[0].dirname,
        templates_hdrs = [],
    ...
    cc_info, _, _ = cc_proto_compile_and_link(
        ctx = ctx,
        deps = deps,
