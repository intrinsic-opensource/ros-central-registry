...
def _force_alwayslink(ctx, direct_cc_infos):
    ...
    for linker_input in merged_cc_info.linking_context.linker_inputs.to_list():
        ...
        if not is_windows:
            if is_darwin:
                if "-Wl,-export_dynamic" not in user_link_flags:
                    user_link_flags.append("-Wl,-export_dynamic")
            elif "-rdynamic" not in user_link_flags:
...
def _c_ros_library(ctx):
    ...
    if ctx.attr.alwayslink or dynamic_mode_off:
        direct_cc_infos, _ = _extract_information(ctx.attr.deps, [
