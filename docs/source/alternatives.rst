...
------------------------------------------------------------
This approach follows the Bazel build-everything-from-source philosophy by providing a collection of ``BUILD.bazel`` files for many existing packages. The tooling downloads a release of the ``ros2.repos`` file, which it transforms into a structured list of package definitions that call a Bazel rule to register the package with a source download location, a collection of patches, and a Bazel ``BUILD`` file. Its main branch tracks ROS 2 Jazzy, with a separate branch for Humble.
...
Key features of this project are:
    ...
    * This project also includes Bazel targets for common ROS 2 command-line tools, like ``ros2 topic``.
...
----------------------------------------------------------
Similar to the first approach, this one also builds everything from source. Their BUILD files are stored in a directory hierarchy. This project appears to be more of a work-in-progress skeleton than something that can be used immediately. This is because some fairly critical packages, like ``sensor_msgs``, don't seem to be included in their implementation. It supports only ROS "Humble", and is tested only on Ubuntu 20.04. Message/IDL generation is still incomplete upstream, and the project otherwise targets C++ only.

`idealworks/bazel-public-registry <https://github.com/idealworks/bazel-public-registry>`__
------------------------------------------------------------------------------------------
Appears to be the only implementation of a ROS distribution as Bazel modules through a custom registry. Packages in the ROS ecosystem are prefixed with ros2. Based on the naming structure, it appears to be pinned to ROS 2 Jazzy and uses a lot of the Bazel providers and aspects from the module `mvukov/rules_ros2 <https://github.com/mvukov/rules_ros2>`__. Unfortunately, there is no supporting tooling around module maintenance, so it is unclear how or when modules will be updated in response to a new upstream ROS release.
